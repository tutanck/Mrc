  ...
  fs.stat(relativePath, err => {
    if (!err) {
      ...
      try {
        ...
        fs.writeFileSync(
          `${relativePath}/${folderName}.js`,
          setupJsFile(folderName)
        );
        fs.writeFileSync(
          `${relativePath}/${folderName}.css`,
          setupCSSFile(folderName)
        );
        fs.writeFileSync(
          `${relativePath}/${folderName}.test.js`,
          setupJsTestFile(folderName)
        );
      } catch (err) {
